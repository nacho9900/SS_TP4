...
every_nth = 1
ax1.errorbar(x1, y1, yerr=[(1 if i%every_nth == 0 else 0)*err for i, err in enumerate(yerr1)], fmt='-', label="Δt = 10⁻¹³")
ax1.errorbar(x2, y2, yerr=[(1 if i%every_nth == 0 else 0)*err for i, err in enumerate(yerr2)], fmt='-', label="Δt = 10⁻¹⁴")
ax1.errorbar(x3, y3, yerr=[(1 if i%every_nth == 0 else 0)*err for i, err in enumerate(yerr3)], fmt='-', label="Δt = 10⁻¹⁵")
...
ax1.legend(handles, labels, loc='best', fontsize=18)
